...
export function DeletePageButton({
  ...
  async function handleDelete(e: MouseEvent<HTMLButtonElement>) {
    ...
    if (!window.confirm(`Biztosan törlöd a(z) "${label}" oldalt? Ez nem vonható vissza.`)) return;

    setDeleting(true);
    try {
      const res = await fetch(`/api/admin/pages/${locale}/${slug}`, { method: "DELETE" });
      if (!res.ok) {
  ...
  if (variant === "icon") {
    return (
      <button
        ...
        disabled={deleting}
        aria-label={`${title || "Oldal"} törlése`}
        title="Törlés"
