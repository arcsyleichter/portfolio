...
import { createEmptyCustomPage, slugify } from "@/lib/builder/defaults";
import { RESERVED_PAGE_SLUGS } from "@/lib/builder/validate";
...
export default function NewCustomPagePage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if ((RESERVED_PAGE_SLUGS as readonly string[]).includes(finalSlug)) {
      setError(`A "${finalSlug}" slug foglalt, válassz másikat.`);
      return;
    }

    setSubmitting(true);

    const existing = await fetch(`/api/admin/pages/${locale}/${finalSlug}`);
    if (existing.ok) {
      setError("Ezzel a slug-gal már létezik oldal ezen a nyelven.");
      setSubmitting(false);
      return;
    }

    const doc = { ...createEmptyCustomPage(locale, finalSlug), title };
    const res = await fetch(`/api/admin/pages/${locale}/${finalSlug}`, {
      method: "PUT",
    ...
    router.push(`/admin/pages/${locale}/${finalSlug}/edit`);
  }

  return (
    <div className="mx-auto max-w-lg px-4 py-12 sm:px-6">
      <h1 className="font-heading text-2xl font-bold">Új egyedi oldal</h1>
